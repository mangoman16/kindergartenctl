...
/**
 * =====================================================================================
 * DATABASE - PDO Connection Manager and Schema Definition
 * =====================================================================================
 *
 * PURPOSE:
 * Singleton class managing the MySQL/MariaDB connection via PDO. Also contains
 * the full database schema definition and installation helpers (create database,
 * run schema, save config file).
 *
 * CONNECTION SETUP:
 * - Config loaded from src/config/database.php (generated by InstallController)
 * - Uses PDO with ERRMODE_EXCEPTION, FETCH_ASSOC, and EMULATE_PREPARES=false
 * - EMULATE_PREPARES=false means named parameters CANNOT be reused in a query
 *   (each :param must appear exactly once). All models follow this constraint.
 *
 * USAGE:
 * ```php
 * $pdo = Database::getInstance(); // Returns singleton PDO or null if not configured
 ...
 *
 * AI NOTES:
 * - getInstance() returns null if no database.php config exists (pre-installation)
 * - All schema DDL uses IF NOT EXISTS for idempotent re-runs
 * - Database name, charset, and collation are validated against whitelists
 *   before interpolation into DDL (prevents SQL injection in CREATE DATABASE)
 * - The games table has BOTH a category_id FK AND a game_categories junction table.
 *   category_id is the "primary" category; game_categories allows additional categories.
 * - saveConfig() generates database.php with var_export() for safe serialization
 *
 * RELATED FILES:
 * - src/config/database.php - Connection credentials (generated, gitignored)
 * - src/core/Model.php - Base model using Database::getInstance()
 * - src/controllers/InstallController.php - Generates database.php during setup
